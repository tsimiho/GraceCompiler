...
public:
    ...
    virtual std::variant<int, char> eval() const override {
        switch (op) {
            ...
            case "-":   return left->eval() - right->eval();
            case "*":   return left->eval() * right->eval();
            case "div": return left->eval() / rigth->eval();
            case "mod": return left->eval() % rigth->eval();
        }
    ...
    virtual void sem() override {
        ...
        switch (op) {
            case "+": case "-": case "*": case "div": case "mod":
                type = TYPE_int; break;
        }
    }
private:
...
public:
    ...
    virtual bool eval() const override {
        switch (op) {
            ...
            case "#":  return left->eval() != right->eval();
            case "<":  return left->eval() < right->eval();
            case ">":  return left->eval() > rigth->eval();
            case "<=": return left->eval() <= rigth->eval();
            case ">=": return left->eval() >= rigth->eval();
        }
        return 42;  // this will never be reached.
