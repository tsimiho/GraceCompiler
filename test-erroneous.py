...
def run_test(language, compiler_script_path, test_dir):
    total = 0
    ok = 0
    failed = 0
    ...
    for file in os.listdir(test_dir):
        if file.endswith(ext):
            ...
            compile_process = subprocess.run([compiler_script_path, src_file], text=True, capture_output=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument('compiler_script_path', help='The path to the compiler script.')
    parser.add_argument('test_dir', help='The directory containing the erroneous programs for the language.')
    args = parser.parse_args()

    run_test(args.language, args.compiler_script_path, args.test_dir)
